    ...
    "general": [
        {
            ...
            "cue": "Keep your back straight. Avoid excessive forward lean.",
            "joint": "trunk_lean",
            "severity": "medium",
        },
        {
            "condition": lambda f: f.get("limb_symmetry_idx", 1) < 0.85,
            "cue": "One side is doing more work than the other. Focus on even movement.",
            "joint": "symmetry",
            "severity": "high",
        },
        {
            "condition": lambda f: abs(f.get("knee_angle_l", 0) - f.get("knee_angle_r", 0)) > 15,
            "cue": "Your knees aren't moving symmetrically. Check your stance width.",
...
def analyze_session_patterns(frames: list[dict], sport: str = "vertical_jump") -> dict:
    ...
    for f in frames:
        for key in ["knee_angle_l", "knee_angle_r", "hip_angle_l", "hip_angle_r", "trunk_lean", "limb_symmetry_idx"]:
            v = f.get(key)
