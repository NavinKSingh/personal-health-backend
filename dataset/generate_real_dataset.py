...
FEEDBACK_TAGS = {
    "vertical_jump": {
        ...
        "good": [
            "Good depth, push harder through the heels.",
            "Solid jump, focus on arm drive.",
            "Good form, work on landing control.",
        ],
        "average": ["Control trunk lean.", "Increase knee flexion depth.", "Work on bilateral symmetry."],
        "poor": [
            "Significant knee valgus detected.",
            "Insufficient depth in countermovement.",
    ...
    "push_up": {
        ...
        "good": [
            "Good form, engage core more.",
            "Solid push-up, watch elbow flare.",
            "Good depth, maintain straight back.",
        ],
    ...
    "snatch": {
        ...
        "good": [
            "Good lift, tighten catch position.",
            "Solid first pull, work on timing.",
            "Good form, improve lockout.",
        ],
    ...
    "sprint": {
        ...
        "good": [
            "Good stride, increase hip drive.",
            "Solid form, relax upper body.",
            "Good mechanics, work on arm drive.",
        ],
    ...
    "javelin": {
        ...
        "good": [
            "Good throw, work on follow-through.",
            "Solid release angle, increase speed.",
            "Good form, improve crossover step.",
        ],
    ...
    "cricket_bat": {
        ...
        "good": [
            "Good shot, maintain head position.",
            "Solid stance, improve backlift.",
            "Good contact, work on follow-through.",
        ],
...
def generate_sport_data(sport, n_per_quality=1250, phases=None, quality_tiers=None):
    ...
    for quality in quality_tiers:
        ...
        for phase in phases:
            ...
            means = np.array(
                [ref["hip"][0], ref["knee"][0], ref["shoulder"][0], ref["elbow"][0], ref["ankle"][0], ref["trunk"][0]],
                dtype=float,
            )
            sds = np.array(
                [ref["hip"][1], ref["knee"][1], ref["shoulder"][1], ref["elbow"][1], ref["ankle"][1], ref["trunk"][1]],
                dtype=float,
            )
            ...
            for i in range(samples_per_phase):
                ...
                lsi = np.clip(
                    np.random.normal(sym_params["mean"], sym_params["sd"]), sym_params["min"], sym_params["max"]
                )
                ...
                if np.random.random() < 0.02:  # 2% occlusion rate
                    np.random.choice(["hip", "knee", "shoulder", "elbow", "ankle"])
                    np.random.choice(["l", "r"])
...
def assign_metadata(df):
    ...
    for session_counter, (_, group) in enumerate(df.groupby(["athlete_id", "sport"]), start=1):
