...
def train_and_evaluate():
    ...
    exclude_cols = {"session_id", "athlete_id", "frame_num", "sport", "phase_label", "quality_label", "feedback_tag"}
    feature_cols = [
        c for c in df.columns if c not in exclude_cols and df[c].dtype in ("float64", "int64", "float32", "int32")
    ]
...
"""
    for cls in classes:
        r = report[cls]
        report_md += (
            f"| {cls} | {r['precision']:.3f} | {r['recall']:.3f} | {r['f1-score']:.3f} | {int(r['support'])} |\n"
        )
