...
def compute_ratios(df: pd.DataFrame) -> pd.DataFrame:
    ...
    upper_avg = (
        (df["shoulder_angle_l"] + df["shoulder_angle_r"]) / 2 + (df["elbow_angle_l"] + df["elbow_angle_r"]) / 2
    ) / 2
...
def compute_composite_scores(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["trunk_alignment_score"] = 1.0 - np.clip((df["trunk_lean"] + df["spine_deviation"] * 2) / 60.0, 0, 1)
    ...
    df["postural_stability"] = 1.0 - np.clip(
        (np.abs(df["trunk_lean"]) / 45.0 + np.abs(df["spine_deviation"]) / 15.0 + np.abs(df["head_forward_pos"]) / 10.0)
        / 3.0,
        0,
        1,
    )
...
def compute_interaction_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["kinetic_chain_idx"] = (knee_avg * hip_avg * df["trunk_lean"].clip(lower=0.1)) / (180 * 180 * 45)
