...
def _compute_progress(athlete_id: str, days: int) -> dict:
    ...
    for s in sessions:
        ...
        sm = s.get("summary") or {}
        score = float(sm.get("avg_form_score") or s.get("avg_form_score") or s.get("form_score") or 0)
        if score > 0:
            by_day[day].append(score)
            all_form_scores.append(score)
        total_reps += int(sm.get("rep_count") or s.get("rep_count") or 0)
        bj = float(sm.get("peak_jump_height_cm") or s.get("peak_jump_height_cm") or s.get("best_jump_height_cm") or 0)
        if bj > best_jump:
            best_jump = bj
        if "bpi_after" in s:
